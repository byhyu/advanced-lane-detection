...
def yellow_white_filter(warped,ry=(0, 255),gy=(0, 255),by=(0, 255),hw=(0, 255),lw=(0, 255),sw=(0, 255),hs=(0, 255),ls=(0, 255),ss=(0, 255)):
    #yello detection
...
def process_frame_pipeline(image):
    image=cv2.cvtColor(image,cv2.COLOR_RGB2BGR)
    dist_pickle = pickle.load(open("camera_cal/wide_dist_pickle.p", "rb"))
    dst = cv2.undistort(image, dist_pickle['mtx'], dist_pickle['dist'], None, dist_pickle['mtx'])
    warped, Minv = transform_perspective(dst)
    ksize = 15  # Choose a larger odd number to smooth gradient measurements
    # gradx = abs_sobel_thresh(warped, orient='x', sobel_kernel=ksize, thresh=(10, 100))
    gradx = abs_sobel_thresh(warped, orient='x', thresh_min=10, thresh_max=100)

    combined_binary = yellow_white_filter(warped, ry=(200, 255), gy=(200, 255), by=(0, 255), hw=(0, 255), lw=(0, 255),sw=(140, 255),hs=(0, 255), ls=(140, 255),ss=(0, 255))
    #combined_all = np.zeros_like(img[:, :, 0])
    #combined_all[((gradx == 1) & (combined_binary == 1))] = 1
    out_img, ploty, left_fitx, right_fitx, left_lane_inds, right_lane_inds, margin, window_img = detect_lanes(combined_binary)
...
def process_test_images(test_images_path = "./test_images"):
    test_images = glob.glob(path.join(test_images_path,"test*.jpg"))
    fig = plt.figure(figsize=(15, 10))
    dist_pickle = pickle.load(open("camera_cal/wide_dist_pickle.p", "rb"))

    # for image_path in test_images:
    for i in (range(len(test_images))):
        print('processing test image number:')
        print(i)
        ...
        ksize = 15  # Choose a larger odd number to smooth gradient measurements
        gradx = abs_sobel_thresh(warped, orient='x', thresh_min=10,thresh_max=100)
        # grady = abs_sobel_thresh(warped, orient='y', thresh_min=20,thresh_max=100)
        ...
        yw_binary = yellow_white_filter(warped, ry=(200, 255), gy=(200, 255), by=(0, 255), hw=(0, 255),
                                              lw=(0, 255), sw=(140, 255), hs=(0, 255), ls=(140, 255), ss=(0, 255))

        combined_all = np.zeros_like(image[:, :, 0])
        combined_all[((gradx == 1) & (yw_binary == 1))] = 1

        out_img, ploty, left_fitx, right_fitx, left_lane_inds, right_lane_inds, margin, window_img = detect_lanes(
            yw_binary) #yw_binary
        left_curverad, right_curverad = get_curvature(ploty, left_fitx, right_fitx)
        print('left_curverad:')
        print(left_curverad)
        ...
        result = project_back(yw_binary, ploty, left_fitx, right_fitx, Minv, image, dst)#yw_binary
        resultRGB = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
    ...
    print('show figure')
    #plt.show()
    fig.savefig('./writeup/back_proj1.png')
    print('finished running process_test_images')


def process_project_video(video_path="project_video.mp4"):
    clip = VideoFileClip(video_path)
    project_clip = clip.fl_image(process_frame_pipeline)
    project_clip.write_videofile('project_video_out.mp4',audio=False)
    print('finished running process_project_video')
    pass

if __name__=="__main__":
    ret, mtx, dist = undistort(directory="camera_cal", ny=6, nx=9)
    process_test_images(test_images_path = "./test_images")
    # process_project_video(video_path="project_video.mp4")
